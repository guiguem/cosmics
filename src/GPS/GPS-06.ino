...
volatile long TCB1CCMP_at_trigger; // 

volatile unsigned long overflowCounter = 0; //Counts overflows in clock (every 4 ms this hapens)
volatile float clkSpeed=62.5;
//(1/cyclesInSecond)*pow(10,9);//clockspeed in ns
...
ISR(TCB1_INT_vect){
  ...
  if (EventFlag==false){ //only record if we have lowered flag from last event
    EventNanosec = (overflowCounter * TCB2.CCMP + TCB1.CNT)*clkSpeed; //#cylces times clockspeed (since second ticked over)
    TCB1CNT_at_trigger = TCB1.CNT;
    TCB1CCMP_at_trigger = TCB1.CCMP;
    EventSec=second;
...
void printTime(unsigned long sec,unsigned long nanosec, bool printDate){
  ...
  Serial.print(TCB1CCMP_at_trigger); Serial.print(':');
